...
async function addEmployee(employeeInfo) {
    ...
    let args = [employeeInfo.firstname, employeeInfo.lastname, employeeInfo.role, employeeInfo.manager];
    const rows = await db.query(query, args);
...
async function addRole(roleInfo) {
    ...
    let args = [title, salary, roleInfo.departmentid];
    const rows = await db.query(query, args);
    ...
    while (!exitLoop) {
        const prompt = await mainPrompt(); switch (prompt.action) {
            case 'Add department': {
                ...
                await showempdepartment();
                break;
            } case 'View all roles': {
                await showrole(); break;
            } case 'Exit': {
                exitLoop = true;
                process.exit(0);
                return;
